...
const allProgress = (material, murajaah, mudabbir) => {
  return Math.round(((material / 125) * 100 + (murajaah / 572) * 100 + (mudabbir / 6236) * 100) / 3);
};
</script>
<template>
  <div>
    <div class="panel pb-1.5 mt-6">
      <div class="flex flex-col mb-4.5">
        <div class="flex items-center flex-wrap">
          <button type="button" v-if="csv" class="btn btn-primary btn-sm m-1" @click="exportTable('csv')">
          ...
          <button type="button" v-if="txt" class="btn btn-primary btn-sm m-1" @click="exportTable('txt')">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 ltr:mr-2 rtl:ml-2">
              <path
                d="M15.3929 4.05365L14.8912 4.61112L15.3929 4.05365ZM19.3517 7.61654L18.85 8.17402L19.3517 7.61654ZM21.654 10.1541L20.9689 10.4592V10.4592L21.654 10.1541ZM3.17157 20.8284L3.7019 20.2981H3.7019L3.17157 20.8284ZM20.8284 20.8284L20.2981 20.2981L20.2981 20.2981L20.8284 20.8284ZM14 21.25H10V22.75H14V21.25ZM2.75 14V10H1.25V14H2.75ZM21.25 13.5629V14H22.75V13.5629H21.25ZM14.8912 4.61112L18.85 8.17402L19.8534 7.05907L15.8947 3.49618L14.8912 4.61112ZM22.75 13.5629C22.75 11.8745 22.7651 10.8055 22.3391 9.84897L20.9689 10.4592C21.2349 11.0565 21.25 11.742 21.25 13.5629H22.75ZM18.85 8.17402C20.2034 9.3921 20.7029 9.86199 20.9689 10.4592L22.3391 9.84897C21.9131 8.89241 21.1084 8.18853 19.8534 7.05907L18.85 8.17402ZM10.0298 2.75C11.6116 2.75 12.2085 2.76158 12.7405 2.96573L13.2779 1.5653C12.4261 1.23842 11.498 1.25 10.0298 1.25V2.75ZM15.8947 3.49618C14.8087 2.51878 14.1297 1.89214 13.2779 1.5653L12.7405 2.96573C13.2727 3.16993 13.7215 3.55836 14.8912 4.61112L15.8947 3.49618ZM10 21.25C8.09318 21.25 6.73851 21.2484 5.71085 21.1102C4.70476 20.975 4.12511 20.7213 3.7019 20.2981L2.64124 21.3588C3.38961 22.1071 4.33855 22.4392 5.51098 22.5969C6.66182 22.7516 8.13558 22.75 10 22.75V21.25ZM1.25 14C1.25 15.8644 1.24841 17.3382 1.40313 18.489C1.56076 19.6614 1.89288 20.6104 2.64124 21.3588L3.7019 20.2981C3.27869 19.8749 3.02502 19.2952 2.88976 18.2892C2.75159 17.2615 2.75 15.9068 2.75 14H1.25ZM14 22.75C15.8644 22.75 17.3382 22.7516 18.489 22.5969C19.6614 22.4392 20.6104 22.1071 21.3588 21.3588L20.2981 20.2981C19.8749 20.7213 19.2952 20.975 18.2892 21.1102C17.2615 21.2484 15.9068 21.25 14 21.25V22.75ZM21.25 14C21.25 15.9068 21.2484 17.2615 21.1102 18.2892C20.975 19.2952 20.7213 19.8749 20.2981 20.2981L21.3588 21.3588C22.1071 20.6104 22.4392 19.6614 22.5969 18.489C22.7516 17.3382 22.75 15.8644 22.75 14H21.25ZM2.75 10C2.75 8.09318 2.75159 6.73851 2.88976 5.71085C3.02502 4.70476 3.27869 4.12511 3.7019 3.7019L2.64124 2.64124C1.89288 3.38961 1.56076 4.33855 1.40313 5.51098C1.24841 6.66182 1.25 8.13558 1.25 10H2.75ZM10.0298 1.25C8.15538 1.25 6.67442 1.24842 5.51887 1.40307C4.34232 1.56054 3.39019 1.8923 2.64124 2.64124L3.7019 3.7019C4.12453 3.27928 4.70596 3.02525 5.71785 2.88982C6.75075 2.75158 8.11311 2.75 10.0298 2.75V1.25Z"
                fill="currentColor"
          ...
          <button type="button" v-if="print" class="btn btn-primary btn-sm m-1" @click="exportTable('print')">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 ltr:mr-2 rtl:ml-2">
              <path
        ...
        <div class="flex md:items-center flex-col md:flex-row md:justify-between md:gap-0 gap-5 pb-2">
          <h5 class="font-semibold text-lg dark:text-white-light flex">
            {{ title }}
            <img v-if="leaderboard" class="ml-2" src="/assets/images/leaderboard.svg" alt="leaderboard" />
          </h5>
          <div class="flex md:justify-between flex-wrap md:flex-nowrap gap-4">
            <input v-model="search" type="text" v-if="searching" class="form-input md:w-auto w-full" placeholder="Cari..." />
            <template v-if="isImport">
              <label for="import-user" class="btn h-10 mb-0 btn-outline-primary cursor-pointer">
            ...
            <router-link v-if="addRoute" :to="addRoute" class="btn btn-primary"
              ><svg class="mr-2" width="24" height="24" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                <g clip-path="url(#clip0_265_1756)">
                  <circle opacity="0.5" cx="8.00016" cy="7.99967" r="6.66667" stroke="#FAFAFA" stroke-width="1.03333" />
                  <path
      ...
      <div class="datatable">
        ...
        >
          <template v-for="slot in slots" v-slot:[slot]="data" :key="slot">
            {{ slot == 'number' ? orderNumber(data.value.index_order) : slot != 'actions' ? data.value : '' }}
            <div class="flex gap-4 items-center" v-if="slot == 'actions'">
                        ...
                        <ul @click="close()">
                          <li>
                            <template v-if="editDataDropdown">
                              <router-link :to="editDataDropdown.to(data)">Edit</router-link>
                            </template>
                          </li>
                          <!-- <li>
                          ...
                          <li>
                            <template v-if="deleteDataDropdown">
                              <button type="button" @click="deleteDataDropdown.click(data)">Hapus</button>
                            </template>
                          </li>
                        </ul>
                      </template>
              ...
              <template v-if="previewData">
                <router-link :to="previewData.to(data)" class="hover:text-primary">
              ...
              <template v-if="deleteData">
                <button type="button" class="hover:text-danger" @click="deleteData.click(data)">
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="w-5 h-5">
              ...
              <template v-if="editDataWithButton">
                <router-link :to="editDataWithButton.to(data)" class="hover:text-info">
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="w-4.5 h-4.5">
                    <path
                      opacity="0.5"
                      d="M22 10.5V12C22 16.714 22 19.0711 20.5355 20.5355C19.0711 22 16.714 22 12 22C7.28595 22 4.92893 22 3.46447 20.5355C2 19.0711 2 16.714 2 12C2 7.28595 2 4.92893 3.46447 3.46447C4.92893 2 7.28595 2 12 2H13.5"
              ...
              <template v-if="previewDataWithButton">
                <router-link :to="previewDataWithButton.to(data)" class="hover:text-primary">
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="w-5 h-5">
                    <path
                      opacity="0.5"
                      d="M3.27489 15.2957C2.42496 14.1915 2 13.6394 2 12C2 10.3606 2.42496 9.80853 3.27489 8.70433C4.97196 6.49956 7.81811 4 12 4C16.1819 4 19.028 6.49956 20.7251 8.70433C21.575 9.80853 22 10.3606 22 12C22 13.6394 21.575 14.1915 20.7251 15.2957C19.028 17.5004 16.1819 20 12 20C7.81811 20 4.97196 17.5004 3.27489 15.2957Z"
              ...
              <template v-if="deleteDataWithButton">
                <button type="button" class="hover:text-danger" @click="deleteDataWithButton.click(data)">
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="w-5 h-5">
                    <path d="M20.5001 6H3.5" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"></path>
                    <path
                      d="M18.8334 8.5L18.3735 15.3991C18.1965 18.054 18.108 19.3815 17.243 20.1907C16.378 21 15.0476 21 12.3868 21H11.6134C8.9526 21 7.6222 21 6.75719 20.1907C5.89218 19.3815 5.80368 18.054 5.62669 15.3991L5.16675 8.5"
              ...
              <template v-if="editDataIcon">
                <router-link :to="editDataIcon.to(data)" class="hover:text-info">
              ...
              <template v-if="previewDataIcon">
                <router-link :to="previewDataIcon.to(data)" class="hover:text-primary">
                  <svg width="24" height="24" viewBox="0 0 21 21" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path
              ...
              <template v-if="deleteDataIcon">
                <button type="button" class="hover:text-danger" @click="deleteDataIcon.click(data)">
                  <svg width="24" height="24" viewBox="0 0 21 21" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M17.5798 5.02148H3.41309" stroke="#E7515A" stroke-width="1.2" stroke-linecap="round" />
                    <path
                      ...
                      stroke-linecap="round"
                    />
                    <path opacity="0.5" d="M8.41309 9.18945L8.82975 13.3561" stroke="#E7515A" stroke-width="1.2" stroke-linecap="round" />
                    <path opacity="0.5" d="M12.5798 9.18945L12.1631 13.3561" stroke="#E7515A" stroke-width="1.2" stroke-linecap="round" />
                    <path
                      opacity="0.5"
          ...
          <template #nameWithRoundedImage1="data">
            <div class="flex items-center gap-2">
              <img
                :style="data.value?.image == null || data.value?.image.length == 0 ? 'background-color: #cbd0d3' : ''"
          ...
          <template #nameWithRoundedImage2="data">
            <div class="flex items-center gap-2" v-if="data.value.user_material != undefined">
              <img
                v-if="data.value.user_material.user?.image != null && data.value.user_material.user?.image.length > 0"
                :src="axios.getUri().replace('/api', '/') + data.value.user_material.user?.image"
                class="w-9 h-9 rounded-md max-w-none"
                alt="user-profile"
              />
              <img v-else src="/assets/images/users.svg" style="background-color: #cbd0d3" class="w-9 h-9 rounded-md max-w-none" alt="user-profile" />
              <div class="font-semibold truncate">{{ data.value.user_material.user?.name }}</div>
            </div>
            <div class="flex items-center gap-2" v-else-if="data.value.fasih_user != undefined">
              <img
                v-if="data.value.fasih_user.user?.image != null && data.value.fasih_user.user?.image.length > 0"
                :src="axios.getUri().replace('/api', '/') + data.value.fasih_user.user?.image"
                class="w-9 h-9 rounded-md max-w-none"
                alt="user-profile"
              />
              <img v-else src="/assets/images/users.svg" style="background-color: #cbd0d3" class="w-9 h-9 rounded-md max-w-none" alt="user-profile" />
              <div class="font-semibold truncate">{{ data.value.fasih_user.user?.name }}</div>
            </div>
            <div class="flex items-center gap-2" v-else-if="data.value.user != undefined">
              <img
                v-if="data.value.user?.image != null && data.value.user?.image.length > 0"
                :src="axios.getUri().replace('/api', '/') + data.value.user?.image"
                class="w-9 h-9 rounded-md max-w-none"
                alt="user-profile"
              />
              <img v-else src="/assets/images/users.svg" style="background-color: #cbd0d3" class="w-9 h-9 rounded-md max-w-none" alt="user-profile" />
              <div class="font-semibold truncate">{{ data.value.user?.name }}</div>
            </div>
          ...
          <template #nameWithRoundedImage3="data">
            <div class="flex items-center gap-2">
              <img
                v-if="data.value.operator &&data.value.operator?.image != null && data.value.operator?.image.length > 0"
                :src="axios.getUri().replace('/api', '/') + data.value.operator?.image"
                class="w-9 h-9 rounded-full max-w-none"
                alt="user-profile"
              />
              <div v-else-if="data.value.operator==null"></div>
              <img v-else src="/assets/images/users.svg" style="background-color: #cbd0d3" class="w-9 h-9 rounded-full max-w-none" alt="user-profile" />
              <div class="font-semibold truncate">{{ data.value.operator?.name }}</div>
            </div>
          </template>
          ...
          <template #faculty="data">
            <span class="badge" :style="`background-color:${data.value.educational_faculty?.color}`" v-if="data.value.educational_faculty != undefined">{{
              data.value.educational_faculty?.name
            }}</span>
            <span
              class="badge"
              ...
              >{{ data.value.user_material.user.educational_faculty?.name }}</span
            >
            <span
              class="badge"
            ...
            <span class="badge" :style="`background-color:${data.value.user?.educational_faculty?.color}`" v-else>{{
              data.value.user?.educational_faculty?.name
            }}</span>
          </template>
          <template #chapter="data">
            <span class="badge" :class="materialColor(data.value.exam.material.chapter)" v-if="data.value.exam != undefined">{{
              data.value.exam.material.chapter == 1 ? 'Hijaiyah' : data.value.exam.material.chapter == 2 ? 'Tahsin' : 'Tajwid'
            }}</span>
            <span class="badge" :class="materialColor(data.value.chapter)" v-else>{{
              data.value.chapter == 1 ? 'Hijaiyah' : data.value.chapter == 2 ? 'Tahsin' : 'Tajwid'
            }}</span>
          </template>
          <template #chapter_progress="data">
            <div class="w-4/5 min-w-[100px] h-2.5 bg-gray-200 dark:bg-dark/40 rounded-full flex">
              <div
                :class="`h-2.5 rounded-full rounded-bl-full text-center text-white text-xs ${materialColor(data.value.chapter)}`"
          ...
          <template #validity="data">
            <div class="flex items-center justify-center">
              <template v-if="rate(data.value.validity) == 5">
                <svg xmlns="http://www.w3.org/2000/svg" width="13" height="14" viewBox="0 0 13 14" fill="none" v-for="i in rate(data.value.validity)" :key="i">
                  <path
              ...
              <template v-if="rate(data.value.validity) == 1">
                <svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 13 13" fill="none" v-for="i in rate(data.value.validity)" :key="i">
                  <path
          ...
          <template #is_reviewed="data">
            <span class="badge badge-outline-info" v-if="!data.value.is_reviewed">Belum Dikoreksi</span>
            <span class="badge badge-outline-danger" v-else-if="!data.value.is_true">Salah</span>
            <span class="badge badge-outline-success" v-else>Benar</span>
          </template>
          <template #all_progress="data">
            <div class="flex items-center">
              <div class="w-3/5 min-w-[100px] h-2.5 bg-gray-200 dark:bg-dark/40 rounded-full flex items-center">
                <div
                  :class="`h-2.5 rounded-full rounded-bl-full text-center text-white text-xs bg-[#805DCA]`"
                  :style="`width:${allProgress(data.value.user_materials_count, data.value.fasih_user_items_count, data.value.mudabbir_recordeds_count)}%;`"
                ></div>
              </div>
              <span class="text-secondary ml-1"
                >{{ allProgress(data.value.user_materials_count, data.value.fasih_user_items_count, data.value.mudabbir_recordeds_count) }}%</span
              >
            </div>
          </template>
          <template #level_material="data">
            <span
              class="badge"
              :class="{
                'bg-success': data.value?.last_user_material == null || data.value?.last_user_material?.material?.chapter == 1,
                'bg-info': data.value?.last_user_material?.material?.chapter == 2,
                'bg-warning': data.value?.last_user_material?.material?.chapter == 3,
              }"
              >{{
          ...
          <template #level_mudabbir="data">
            <span class="badge" :class="materialColor(data.value.is_fasih3 ? 3 : data.value.is_fasih2 ? 2 : 1)">{{
              data.value.is_fasih3 ? 'Level 3' : data.value.is_fasih2 ? 'Level 2' : 'Level 1'
            }}</span>
          </template>
          ...
          <div class="flex min-h-full items-center justify-center px-4 py-8">
            ...
            >
              <DialogPanel class="panel border-0 p-0 rounded-lg overflow-hidden w-full max-w-lg text-black dark:text-white-dark">
                ...
                <div class="text-lg font-bold ltr:pl-5 rtl:pr-5 pt-3 pb-1 ltr:pr-[50px] rtl:pl-[50px]">Filter</div>
