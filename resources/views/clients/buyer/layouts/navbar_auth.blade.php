<div class="topbar">
    <div class="container-topbar w-100">
        ...
        <div class="info-topbar w-100 text-center d-none d-xl-block"><a class="font-xs color-brand-3"
                href="{{ route('buyer.home') }}{{ Auth::user() ?  '?auth='.base64_encode(Auth::user()->uid) : '' }}" target="_blank">Download Aplikasi PIS Shop Global</a></div>
    </div>
</div>
</div>
    ...
    <div class="container">
        <div class="main-header">
            <div class="header-left">
                <div class="header-logo logo-auth"><a class="d-flex" href="{{ route('buyer.home') }}{{ Auth::user() ?  '?auth='.base64_encode(Auth::user()->uid) : '' }}"><img
                ...
                <div class="header-nav">
                    <nav class="nav-main-menu d-none d-xl-block">
                        <ul class="main-menu">
                            <li><a class="@yield('home')" href="{{ route('buyer.home') }}{{ Auth::user() ?  '?auth='.base64_encode(Auth::user()->uid) : '' }}">Beranda</a></li>
                            {{-- @guest
                                <li><a class="@yield('login')" href="{{ route('buyer.login') }}">Masuk</a></li>
                            @endguest --}}
                            <li><a class="@yield('allProduct')" href="{{ route('buyer.allGridProduct') }}{{ Auth::user() ?  '?auth='.base64_encode(Auth::user()->uid) : '' }}">Semua
                                    Produk</a>
                            </li>
                            <li><a class="@yield('article')" href="{{ route('buyer.allArticle') }}{{ Auth::user() ?  '?auth='.base64_encode(Auth::user()->uid) : '' }}">Artikel</a>
                            </li>
                        </ul>
                    </nav>
                        ...
                        @if (Auth::guard('web')->user()->is_seller == 1)
                            <div class="d-lg-inline-block d-none">
                                <a class="font-lg btn btn-buy d-inline-block" style="padding:7px 10px !important;"
                                    href="{{ route('dashboardSeller.dashboard') }}{{ Auth::user() ?  '?auth='.base64_encode(Auth::user()->uid) : '' }}">
                                    Toko Saya
                                </a>
                            </div>
                        @endif
                                ...
                                <div class="cart-total d-none">
                                    ...
                                    <div class="row mt-15">
                                        <div class="col-6 text-start"><a class="btn btn-cart w-auto"
                                                href="{{ route('buyer.cart') }}{{ Auth::user() ?  '?auth='.base64_encode(Auth::user()->uid) : '' }}">Keranjang</a></div>
                        ...
                        <div class="d-inline-block box-dropdown-cart"><span
                            ...
                            <div class="dropdown-account">
                                <ul>
                                    ...
                                    <li><a href="{{ route('dashboard.myOrder') }}{{ Auth::user() ?  '?auth='.base64_encode(Auth::user()->uid) : '' }}">Pesanan ku</a></li>
                                    <li><a href="{{ route('buyer.wishlist') }}{{ Auth::user() ?  '?auth='.base64_encode(Auth::user()->uid) : '' }}">Wishlist</a></li>
                                    @if (Auth::guard('web')->user()->is_seller == 0)
                                        <li><a href="{{ route('dashboard.settings') }}{{ Auth::user() ?  '?auth='.base64_encode(Auth::user()->uid) : '' }}">Daftar Toko</a></li>
                                    @else
                                        <li><a href="{{ route('dashboardSeller.dashboard') }}{{ Auth::user() ?  '?auth='.base64_encode(Auth::user()->uid) : '' }}">Toko Saya</a></li>
                                    @endif
                                    <li><a href="{{ route('dashboard.settings') }}{{ Auth::user() ?  '?auth='.base64_encode(Auth::user()->uid) : '' }}">Pengaturan</a></li>
                                    <li><a href="{{ route('logout') }}{{ Auth::user() ?  '?auth='.base64_encode(Auth::user()->uid) : '' }}">Keluar Akun</a></li>
                                </ul>
                            </div>
                        </div>
                    </div>
                ...
                @guest
                    <div class="discount d-xl-flex d-none gap-2">
                        <a class="@yield('login') btn btn-buy" href="{{ route('buyer.login') }}{{ Auth::user() ?  '?auth='.base64_encode(Auth::user()->uid) : '' }}">Masuk</a>
                        <a class="@yield('register') btn btn-cart" href="{{ route('buyer.register') }}{{ Auth::user() ?  '?auth='.base64_encode(Auth::user()->uid) : '' }}">Daftar</a>
                    </div>
                @endguest
            </div>
        </div>
...
<div class="mobile-header-active mobile-header-wrapper-style perfect-scrollbar">
    <div class="mobile-header-wrapper-inner">
        <div class="mobile-header-content-area position-absolute">
            <div class="mobile-logo"><a class="d-flex" href="{{ route('buyer.home') }}{{ Auth::user() ?  '?auth='.base64_encode(Auth::user()->uid) : '' }}"><img alt="Logo Pishop"
                ...
                <div class="mobile-menu-wrap mobile-header-border">
                    <nav class="mt-15">
                        <ul class="mobile-menu font-heading">
                            <li><a class="@yield('home')" href="{{ route('buyer.home') }}{{ Auth::user() ?  '?auth='.base64_encode(Auth::user()->uid) : '' }}">Beranda</a></li>
                            @guest
                                <li><a class="@yield('login')" href="{{ route('buyer.login') }}{{ Auth::user() ?  '?auth='.base64_encode(Auth::user()->uid) : '' }}">Masuk</a></li>
                            @endguest
                            <li><a class="@yield('register')" href="{{ route('buyer.register') }}{{ Auth::user() ?  '?auth='.base64_encode(Auth::user()->uid) : '' }}">Daftar</a></li>
                            <li><a class="@yield('allProduct')" href="{{ route('buyer.allGridProduct') }}{{ Auth::user() ?  '?auth='.base64_encode(Auth::user()->uid) : '' }}">Semua
                                    Produk</a></li>
                            <li><a class="@yield('article')" href="{{ route('buyer.allArticle') }}{{ Auth::user() ?  '?auth='.base64_encode(Auth::user()->uid) : '' }}">Artikel</a>
                            </li>
                    ...
                    <div class="mobile-account">
                        <div class="mobile-header-top">
                            <div class="user-account">
                                <a href="{{ route('dashboard.settings') }}{{ Auth::user() ?  '?auth='.base64_encode(Auth::user()->uid) : '' }}"><img width="80px" height="80px"
                        ...
                        <ul class="mobile-menu">
                            ...
                            <li><a href="{{ route('buyer.wishlist') }}{{ Auth::user() ?  '?auth='.base64_encode(Auth::user()->uid) : '' }}">Wishlist</a></li>
                            @if (Auth::guard('web')->user()->is_seller == 0)
                                <li><a href="{{ route('dashboardSeller.profile') }}{{ Auth::user() ?  '?auth='.base64_encode(Auth::user()->uid) : '' }}">Daftar Toko</a></li>
                            @else
                                <li><a href="{{ route('dashboardSeller.dashboard') }}{{ Auth::user() ?  '?auth='.base64_encode(Auth::user()->uid) : '' }}">Toko Saya</a></li>
                            @endif
                            <li><a href="{{ route('dashboard.settings') }}{{ Auth::user() ?  '?auth='.base64_encode(Auth::user()->uid) : '' }}">Pengaturan</a></li>
        ...
        <div class="mobile-header-content-area position-absolute">
            ...
            <div class="perfect-scroll" style="height:100% !important">
                <div class="mobile-menu-wrap mobile-header-border">
                    <nav class="mt-15">
                        <ul class="mobile-menu font-heading">
                            ...
                            @guest
                                <li><a class="@yield('login')" href="{{ route('buyer.login') }}{{ Auth::user() ?  '?auth='.base64_encode(Auth::user()->uid) : '' }}">Masuk</a></li>
                                <li><a class="@yield('register')" href="{{ route('buyer.register') }}{{ Auth::user() ?  '?auth='.base64_encode(Auth::user()->uid) : '' }}">Daftar</a>
                                </li>
                            @endguest
                            <li><a class="@yield('allProduct')" href="{{ route('buyer.allGridProduct') }}{{ Auth::user() ?  '?auth='.base64_encode(Auth::user()->uid) : '' }}">Semua
                                    Produk</a></li>
                ...
                @auth
                    <div class="mobile-account">
                        <div class="mobile-header-top">
                            <div class="user-account"><a href="{{ route('dashboard.settings') }}{{ Auth::user() ?  '?auth='.base64_encode(Auth::user()->uid) : '' }}"><img width="80px"
                                        height="80px"
                                        src="{{ Auth::guard('web')->user() && Auth::guard('web')->user()->image ? Auth::guard('web')->user()->image ?? asset('ecom/imgs/users.svg') : asset('ecom/imgs/users.svg') }}"
                                        alt="akun {{ Auth::guard('web')->user()->name ?? '' }}"></a>
                                <div class="content">
...
@push('importjs')
    ...
    <script>
        ...
        var baseDetailProductUrl = "{{ $baseDetailProductUrl }}{{ Auth::user() ?  '?auth='.base64_encode(Auth::user()->uid) : '' }}";
    </script>
    <script>
            ...
            if (carts) {
                ...
                for (var i = 0; i < Math.min(2, cart.length); i++) {
                    ...
                    <div class="cart-info">
                        ...
                        href="${url}{{ Auth::user() ?  '?auth='.base64_encode(Auth::user()->uid) : '' }}">
                            ${element.name}
                        </a>
