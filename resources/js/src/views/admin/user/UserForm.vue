<template>
  <div>
    ...
    >
      <li>
        <router-link
          ...
          >Master Data Pengguna</router-link
        >
      </li>
    ...
    <div class="pt-3">
      ...
      >
        <div
          class="w-full lg:w-4/4 border border-[#ebedf2] dark:border-[#191e3a] rounded-md p-4 mb-5 bg-white dark:bg-[#0e1726]"
        >
          <p class="pb-3 text-lg font-semibold">Data Pengguna</p>
          <div class="flex-1">
            <div class="grid grid-cols-1 gap-5">
              <div class="flex flex-col gap-2">
                <label class="w-full md:mb-0 font-semibold" for="name"
                  >Nama</label
                >
                <input
              ...
              <div class="flex flex-col gap-2">
                <label class="w-full md:mb-0 font-semibold" for="phone_number"
                  >Nomor Telepon</label
                >
                <input
                  class="md:w-4/4 form-input"
              ...
              <div class="gap-2">
                <label class="w-full md:mb-0 font-semibold" for="is_seller"
                  >Status Penjual</label
                >
                <input
                  class="scale-110"
                  id="is_seller"
                ...
                />
                <span class="ml-2">{{
                  isSeller ? "Seller" : "Bukan Seller"
                }}</span>
              </div>
              <div class="gap-2">
                <label
                  class="w-full md:mb-0 font-semibold"
                  for="is_seller_active"
                  >Apakah Penjual Aktif?</label
                >
                <input
                  class="scale-110"
                  id="is_seller_active"
                  type="checkbox"
                  v-model="isSellerActive"
                />
                <span class="ml-2">{{
                  isSellerActive ? "Aktif" : "Tidak Aktif"
                }}</span>
              </div>
              <div class="flex flex-col gap-2">
                <label class="w-full md:mb-0 font-semibold" for="seller_name"
                  >Nama Penjual</label
                >
                <input
                  class="md:w-4/4 form-input"
              ...
              <div class="flex flex-col gap-2">
                ...
                <input
                  class="md:w-4/4 form-input"
                  id="seller_fee_amount"
                  type="number"
                  min="0"
                  placeholder="Masukkan jumlah biaya penjual (contoh : 1000)"
                  v-model="form.seller_fee_amount"
                  @change="touchFeeData('amount')"
                  :disabled="
                    form.seller_fee_percentage != null &&
                    form.seller_fee_percentage > 0
                  "
                />
              </div>
              <div class="flex flex-col gap-2">
                <label
                  ...
                  >Persentase Fee Penjual (%)</label
                >
                <input
                  class="md:w-4/4 form-input"
                  id="seller_fee_percentage"
                  type="number"
                  min="0"
                  placeholder="Masukkan persentase (%) biaya penjual"
                  @change="touchFeeData('percentage')"
                  :disabled="
                    form.seller_fee_amount != null && form.seller_fee_amount > 0
                  "
                  v-model="form.seller_fee_percentage"
                />
              </div>

              <div class="flex flex-col gap-2">
                <label class="w-full md:mb-0 font-semibold" for="password"
                  >Kata Sandi (Abaikan jika tak diperlukan, Isi untuk mengubah
                  kata sandi)</label
                >
                <input
                  class="md:w-4/4 form-input"
              ...
              <div class="flex mt-4">
                <button
                  type="submit"
                  @submit.prevent="handleSubmit"
                  class="btn btn-success lg:w-2/8 btn-lg mr-2"
                >
                  <svg
                    width="24"
                    height="24"
                ...
                <button
                  type="button"
                  @click="deleteUser"
                  class="btn btn-danger btn-lg lg:w-2/8"
...
onMounted(async () => {
  ...
  if (route.params.id) {
    form.value = (await axios.get(`/admin/users/${route.params.id}`)).data;
    isSeller.value = Boolean(form.value.is_seller);
    isSellerActive.value = Boolean(form.value.is_seller_active);
  }
});
...
const getUser = () => {
  ...
  // axios get into `admin/categories/all-categories`
  axios
...
const touchFeeData = (type) => {
  if (type == "amount") {
    form.value.seller_fee_percentage = 0;
  } else if (type == "percentage") {
    form.value.seller_fee_amount = 0;
  }
};

const handleSubmit = async () => {
  form.value.is_seller = isSeller.value;
  form.value.is_seller_active = isSellerActive.value;
  let fform = globalComponents.dataToFormData(form.value);
  store.isShowMainLoader = true;
  axios
    .post(
      "/admin/users/update/" + (route.params.id ? route.params.id : ""),
      fform
    )
    .then((res) => {
...
const deleteUser = () => {
  ...
  swalWithBootstrapButtons
    ...
    .then((result) => {
      if (result.value) {
        axios.delete(`/admin/users/${form.value.id}`).then((res) => {
          toast.fire("Pengguna berhasil dihapus.");
