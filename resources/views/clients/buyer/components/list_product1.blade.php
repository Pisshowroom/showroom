<div class="card-grid-style-3">
    <div class="card-grid-inner">
        ...
        <div class="image-box">
            ...
            @auth
                <a
                    href="{{ route('buyer.detailProduct', ['slug' => $prd->slug ?? 'sd']) }}{{ Auth::check() && preg_match('/Chrome/i', request()->header('User-Agent')) ? '?auth=' . base64_encode(Auth::user()->uid) : '' }}">
                @endauth
                @guest
        ...
        <div class="info-right">
            @if ($prd?->seller && $prd?->seller?->seller_slug)
                <a class="font-xs color-gray-500 line-1 text-start"
                    href="{{ route('buyer.detailSeller', ['slug' => $prd?->seller?->seller_slug]) }}{{ Auth::check() && preg_match('/Chrome/i', request()->header('User-Agent')) ? '?auth=' . base64_encode(Auth::user()->uid) : '' }}">
