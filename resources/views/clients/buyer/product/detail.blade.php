                ...
                <div class="container">
                    <ul class="breadcrumb">
                        ...
                        <li><a class="font-xs color-gray-500"
                                href="{{ route('buyer.detailProduct', ['slug' => $product->slug]) }}{{ Auth::check() && preg_match('/Chrome/i', request()->header('User-Agent')) ? '?auth=' . base64_encode(Auth::user()->uid) : '' }}">Detail
                                Produk</a>
                        </li>
                    </ul>
                </div>
                    ...
                    <div class="col-lg-7">
                        ...
                        <div class="row align-items-center">
                            <div class="col-lg-4 col-md-4 col-sm-3 mb-mobile">
                                @if ($product?->seller && $product?->seller?->seller_slug)
                                    <a class="byAUthor color-gray-900 font-xs font-medium"
                                        href="{{ route('buyer.detailSeller', ['slug' => $product?->seller?->seller_slug]) }}{{ Auth::check() && preg_match('/Chrome/i', request()->header('User-Agent')) ? '?auth=' . base64_encode(Auth::user()->uid) : '' }}">
                            ...
                            <div class="col-lg-8 col-md-8 col-sm-9 text-start text-sm-end">
                                <a class="mr-20"
                                    href="{{ Auth::check() && preg_match('/Chrome/i', request()->header('User-Agent')) ? route('buyer.wishlist') . '?auth=' . base64_encode(Auth::user()->uid) : route('buyer.login') }}"><span
                                    ...
                                    <div class="row">
                                        <div class="col-lg-8">
                                            <form class="form-comment"
                                                action="{{ route('buyer.addReview') }}{{ Auth::check() && preg_match('/Chrome/i', request()->header('User-Agent')) ? '?auth=' . base64_encode(Auth::user()->uid) : '' }}"
                                                method="POST">
                                                @csrf
                                                <div class="row">
